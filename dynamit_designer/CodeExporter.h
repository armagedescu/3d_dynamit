...
private:
    ...
    static std::wstring generateStandaloneApp(const ShapeConfig& config, bool includeNormals)
    {
        ...
        code << L"#include <callbacks.h>\n";
        code << L"#include <geometry.h>\n\n";
        ...
        code << L"using namespace dynamit;\n";
        code << L"using namespace dynamit::builders;\n";
        code << L"using namespace dynamit::geo;\n\n";

        code << L"// Global camera\n";
        code << L"Camera camera;\n";

        if (includeNormals)
        {
            code << L"bool showNormals = false;\n";
        }
        ...
        if (includeNormals)
        {
            ...
            code << L"            showNormals = !showNormals;\n";
        }

        code << L"    }\n";
        code << L"    handleKeyboard(window, key, scancode, action, mods, camera);\n";
        code << L"}\n\n";
        ...
        code << L"    glfwSetCursorPosCallback(window, [](GLFWwindow* w, double x, double y) { handleMouseMove(w, x, y, camera); });\n";
        ...
        code << L"    // Initialize camera\n";
        code << L"    camera.setPosition(0.0f, 0.0f, 3.0f);\n";
        code << L"    camera.lookAt(0.0f, 0.0f, 0.0f);\n\n";
        ...
        code << L"    // Render loop\n";
        code << L"    double lastTime = glfwGetTime();\n";
        code << L"    while (!glfwWindowShouldClose(window))\n";
        code << L"    {\n";
        code << L"        // Delta time\n";
        code << L"        double currentTime = glfwGetTime();\n";
        code << L"        float deltaTime = static_cast<float>(currentTime - lastTime);\n";
        code << L"        lastTime = currentTime;\n\n";

        code << L"        // Update camera\n";
        code << L"        updateCamera(window, camera, deltaTime);\n\n";
        ...
        code << L"        glm::mat4 projection = glm::perspective(glm::radians(45.0f), \n";
        code << L"                                                (float)width / (float)height, \n";
        code << L"                                                0.1f, 100.0f);\n";
        code << L"        glm::mat4 view = camera.getViewMatrix();\n";
