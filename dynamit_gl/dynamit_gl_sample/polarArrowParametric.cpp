...
int main_polarArrowParametric()
{
    ...
    }else  if (buildHeart)
    {
        builder.doubleCoated()
            ...
            .reversed(false).buildConeIndexed(verts, norms, indices, arrowTipScale, arrowTipTranslate, rotation_x_mat4(-M_PI / 2))
            .reversed(false).buildCylinderIndexed(verts, norms, indices, arrowShaftScale, arrowShaftTranslate, rotation_x_mat4(-M_PI / 2))
            .reversed(false).buildConeIndexed(verts, norms, indices, arrowTipScale, arrowTipTranslate, rotation_y_mat4(-M_PI / 2))
            .reversed(false).buildCylinderIndexed(verts, norms, indices, arrowShaftScale, arrowShaftTranslate, rotation_y_mat4(-M_PI / 2))
            .reversed(false).buildConeIndexed(verts, norms, indices, arrowTipScale, arrowTipTranslate, rotation_x_mat4(M_PI))
