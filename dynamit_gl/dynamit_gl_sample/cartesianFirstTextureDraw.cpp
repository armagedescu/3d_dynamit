...
int main_cartesianFirstTextureDraw()
{
    ...
    Builder::cartesian()
        ...
        //.buildCylinderIndexedWithColorAndTexCoords(verts, norms, colors, texCoords, indices)
        //.buildCylinderIndexedWithColorAndTexCoords(verts, norms, colors, texCoords, indices)
        ;
    ...
    // Load texture and create textured shape
    GLuint crateTexture = LoadTexture("bitmaps/crate.jpg");

    Dynamit texturedCube;
    texturedCube
        .withVertices3d(verts)
        .withNormals3d(norms)
        .withTexCoords2d(texCoords)
        .withTexture(crateTexture, 0, "texture0")
        .withConstLightDirection({ -0.577f, -0.577f, 0.577f })
        .withTransformMatrix4f();

    // Or load directly from path:
    Dynamit texturedShape;
    texturedShape
        .withVertices3d(verts)
        .withTexCoords2d(texCoords)
        .withNormals3d(norms)
        ...
        .withTexture("bitmaps/wood.png", 0, "diffuseMap")
        .withTexture("bitmaps/wood_normal.png", 1, "normalMap");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        texturedShape.transformMatrix4f(mat4Transform);
        texturedShape.drawTrianglesIndexed();
...
#include "enabler.h"
#ifdef __MAIN_CARTESIAN_FIRST_TEXTURE_DRAW_CPP__
int main() { return main_cartesianFirstTextureDraw(); }
#endif