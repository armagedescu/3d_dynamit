...
const wchar_t* TerrainTessellated::defTerrainImgPath = L"bitmaps/heightmap.bmp";

TerrainTessellated::TerrainTessellated (const wchar_t* heigthsMapPath)
	: TerrainTessellated (heigthsMapPath,
		"shaders/tessellation/terrainIndexed.tcs", "shaders/tessellation/terrainIndexed.tes",
		"shaders/tessellation/terrainIndexed.vs",  "shaders/tessellation/terrainIndexed.fs")
{
}
TerrainTessellated::TerrainTessellated (const char* controlShader, const char* evaluationShader, const char* vertexPath, const char* fragmentPath)
	: TerrainTessellated (defTerrainImgPath, controlShader, evaluationShader, vertexPath, fragmentPath)
{
}
TerrainTessellated::TerrainTessellated(const wchar_t* heigthsMapPath, const char* controlShader, const char* evaluationShader, const char* vertexPath, const char* fragmentPath)
	: terrainImgPath (heigthsMapPath),
	pos (0.0f, 0.0f, 0.0f),
	Tess (controlShader, evaluationShader, vertexPath, fragmentPath)
{
	build();
}

int TerrainTessellated::fillHeightMapBuffer(float size, float h)
{
	if (!vertexes.empty()) return 0;
	std::vector<std::vector<float>> heights;
	int bpr = HeigthMapFromImg(terrainImgPath, heights, 10);
	if (bpr == -1) return -1;

	unsigned int height, width;
	height = heights.size() - 1; width = heights[0].size() - 1;
	//height = 70;
	//width  = 70;

	int coordsCount = height * width * 3; //no triangles, 3 points per vertex, the triangles are on indexes
	...
	//indexes
	int rectanglesCount = (height - 1) * (width - 1);
	...
	for (int i = height, i0 = 0; i0 < height - 1; i++, i0++)
	{
		for (int j = 0; j < width - 1; j++)
		{
...
void TerrainTessellated::drawInit()
{
	using config::camera;
...
void TerrainTessellated::drawInit(glm::mat4& model, glm::mat4& view, glm::mat4& projection, const glm::vec4& color)
{
	glUseProgram(*this);
