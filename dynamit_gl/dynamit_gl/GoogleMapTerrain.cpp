...
using std::cout;
using std::endl;
const wchar_t* GoogleMapTerrain::defTerrainImgPath = L"bitmaps/heightmap.bmp";

GoogleMapTerrain::GoogleMapTerrain(const wchar_t* heigthsMapPath) :
	GoogleMapTerrain(heigthsMapPath, "shaders/googleMapTerrain.vs", "shaders/googleMapTerrain.fs")
{
}
GoogleMapTerrain::GoogleMapTerrain(const char* vertexPath, const char* fragmentPath) :
	GoogleMapTerrain(defTerrainImgPath, vertexPath, fragmentPath)
{}

GoogleMapTerrain::GoogleMapTerrain(const wchar_t* heigthsMapPath, const char* vertexPath, const char* fragmentPath) :
...
int GoogleMapTerrain::fillHeightMapBuffer(float size, float h)
{
	...
	for (size_t i = 0; i < width; i++)
	{
		for (size_t j = 0; j < length; j++)
		{
			...
			resize3(x0, y0, z0, rfi, maxheight, rfj); offset3(x0, y0, z0, -1.f, bottom, -1.f);
...
void GoogleMapTerrain::drawInit()
{
	using config::camera;
...
void GoogleMapTerrain::drawInit(glm::mat4& model, glm::mat4& view, glm::mat4& projection, const glm::vec4& color)
{
	glUseProgram(*this);
...
void GoogleMapTerrain::draw()
{
	glUseProgram(*this);    //set shader program to drao
	glBindVertexArray(vao); //set object to draw
	glDrawArrays(GL_TRIANGLES, 0, vertexes.size() / 3); //draw
}
