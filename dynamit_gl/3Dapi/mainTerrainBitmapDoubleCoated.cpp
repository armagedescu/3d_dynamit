#include "enabler.h"

#include <GL/glew.h>
#include <config.h>
#include <cstdlib>
#include <iostream>

#define _USE_MATH_DEFINES
#include <corecrt_math_defines.h>
#include <cmath>

#include <glm/glm.hpp>                  //basic glm math functions
#include <glm/gtc/matrix_transform.hpp> //matrix functions
#include <glm/gtc/type_ptr.hpp>         //convert glm types to opengl types

#include <Terrain.h>

using namespace std;

int main_terrain_bitmap_doublecoated()
{
	using namespace std;
	srand(time(NULL));
	GLFWwindow* window = openglWindowInit();
	if (!window)
		return -1;

	Terrain terrain("shaders/terrain.vs", "shaders/terrain.fs");
	if (!terrain)
	{
		char infoLog[512];
		glGetProgramInfoLog(terrain, 512, NULL, infoLog);
		std::cout << "shaders failed\n" << infoLog << std::endl;
		return -1;
	}

	glEnable(GL_CULL_FACE);
	glEnable(GL_DEPTH_TEST);

	glm::vec3 pos(0.0f, 0.0f, 0.0f);


	//config::camera.position.y =   2;
	//config::camera.position.z = 1.5;
	//config::camera.pitch      = -50;
	config::camera.yaw = -90.f;
	config::camera.updateVectors();

    //mesh visualizer
	//glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	//glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	//glPolygonMode(GL_FRONT_AND_BACK, GL_POINT);
	//glFrontFace(GL_CW);

	while (!glfwWindowShouldClose(window))
	{
		using config::camera;
		using config::windowWidth;
		using config::windowHeight;

		processInputs(window);

		glClearColor(0.f, 0.f, 1.f, 1.0f);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

		using config::camera;
		using config::windowWidth;
		using config::windowHeight;
		glm::mat4 model = glm::mat4(1.0);
		model = glm::translate(model, {0, 0, 0});
		//cout << M_PI << endl;
		//model = glm::rotate(model, (float) M_PI_4 / 2.f, glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, (float) glfwGetTime(), glm::vec3(0.0f, 1.0f, 0.0f));
		glm::mat4 view = camera.view();
		//glm::mat4 view = glm::mat4(1.0);
		glm::mat4 projection = glm::mat4(1.0f);
		projection = glm::perspective(glm::radians(camera.zoom), (float)windowWidth / windowHeight, 0.1f, 100.0f);
		//projection = glm::perspective(glm::radians(camera.zoom), (float)windowWidth / windowHeight, 100.0f, 0.1f);
		terrain.drawInit(model, view, projection, glm::vec4(1, 0, 0, 1));

		//terrain.drawInit();
		terrain.draw();

		glfwSwapBuffers(window);
		glfwPollEvents();
	}

	glfwTerminate();
	return 0;
}

#include "enabler.h"
#ifdef __MAIN_TERRAIN_BITMAP_DOUBLECOATED_CPP__
int main() { return main_terrain_bitmap_doublecoated(); }
#endif